...
import {
	forEachElem, apiUrl, fetchWithRetry, showError, chartVisibility, formatPlaceName,
} from './utils.mjs';
import {
...
import * as Outlook from './spcoutlook/index.mjs';
...
import getHourly from './forecast/hourly.mjs';
...
const getPlace = (isRetry) => {
	...
	if (savedPlace.followMe) {
		...
		navigator.geolocation.getCurrentPosition(
			(position) => {
				positionReceived(position, savedPlace);
			},
			(e) => {
				// geoloc error
...
document.addEventListener('DOMContentLoaded', () => {
	...
	// register the getPlace callback (avoids a dependency cycle from observations.mjs importing this module)
	Observations.init(getPlace);
